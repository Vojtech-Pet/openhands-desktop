...
class ConversationController(QObject):
    ...
    def pause(self) -> None:
        """Soft stop: finish the current LLM call, then pause the agent."""
        if self._sandbox is None:
            self.error_occurred.emit("No active conversation to pause")
            return
        self._spawn(self._pause())
    ...
    async def _pause(self) -> None:
        ...
        try:
            ...
            self.error_occurred.emit(f"Could not pause the agent: {exc}")
