...
class _IntegrationsPage(QWidget):
    ...
    def _on_disconnect_all(self) -> None:
        ...
        if reply != QMessageBox.StandardButton.Yes:
            return
        _run_async(
            self, self._client.unset_git_provider_tokens(), lambda _: self._on_all_disconnected(),
...
class _SecretsPage(QWidget):
    ...
    def _on_edit_clicked(self, secret: dict) -> None:
        dialog = _SecretFormDialog(self, editing=secret)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            values = dialog.values()
            _run_async(
                self,
                self._client.update_secret(secret["name"], values["name"], values["description"]),
                lambda _: self._reload(),
                error_title="Failed to update secret",
            )
